...
class PDFToMarkdownConverter:
    ...
    Also creates a plain text version for easier AI analysis.
    """
    ...
    def __init__(self, input_path, output_dir=None):
        ...
        self.output_md_path = os.path.join(self.output_folder, f"{self.base_filename}.md")
        self.output_txt_path = os.path.join(self.output_folder, f"{self.base_filename}.txt")
    ...
    def _convert_table_to_text(self, table_data):
        """
        Convert table data to plain text format (tab-separated).
    ...
    def convert(self):
        """
        Main conversion method to convert PDF to Markdown and Plain Text.
        
        Returns:
            tuple: (markdown_content, text_content)
        """
        try:
            ...
            # Add YAML frontmatter (only for markdown)
            frontmatter = {
            ...
            # Add header for text version
            text_content.append("="*60)
            text_content.append(f"Title: {self.metadata['title']}")
            text_content.append(f"Author: {self.metadata['author']}")
            text_content.append(f"Pages: {self.metadata['pages']}")
            text_content.append(f"Source: {self.metadata['source_file']}")
            text_content.append(f"Converted: {self.metadata['created']}")
            text_content.append("="*60)
            text_content.append("")
            ...
            for page_num in range(self.doc.page_count):
                ...
                for element in elements:
                    ...
                    if text:
                        if element["heading_level"] > 0:
                            # Markdown format
                            heading = "#" * (element["heading_level"] + 2)
                            markdown_content.append(f"{heading} {text}")
                            
                            # Text format (add extra line for emphasis)
                            text_content.append("")
                            text_content.append(text)
                            text_content.append("")
                        else:
                            # Both get the same plain text
                ...
                if tables:
                    ...
                    for table in tables:
                        if table:
                            ...
                            # Text version
                            table_txt = self._convert_table_to_text(table)
                            if table_txt:
                                text_content.append("[TABLE]")
                                text_content.append(table_txt)
                                text_content.append("")
                ...
                if image_count > 0:
                    ...
                    text_content.append(f"[{image_count} image(s) on this page]")
                    text_content.append("")
    ...
    def save(self, markdown_content, text_content):
        """
        Save both Markdown and Text content to files.
        
        Args:
            markdown_content (str): Markdown content to save
            text_content (str): Plain text content to save
        """
        try:
            # Save markdown file
    ...
    def get_output_info(self):
        ...
        return {
            ...
            "markdown_file": self.output_md_path,
            "text_file": self.output_txt_path
        }


def convert_single_pdf(pdf_path, output_dir=None):
    """
    Convert a single PDF file to Markdown and Text.
    ...
    try:
        ...
        markdown_content, text_content = converter.convert()
        converter.save(markdown_content, text_content)
        
        return {
            "success": True,
            "filename": os.path.basename(pdf_path),
            "output_md": converter.get_output_info()["markdown_file"],
            "output_txt": converter.get_output_info()["text_file"],
            "tables": converter.total_tables,
            "images": converter.total_images,
            "md_lines": len(markdown_content.split('\n')),
            "txt_lines": len(text_content.split('\n')),
            "md_size": len(markdown_content),
            "txt_size": len(text_content)
        }
    except Exception as e:
        return {
            "success": False,
...
def convert_batch(source_dir=None, output_dir=None):
    ...
    for i, pdf_file in enumerate(pdf_files, 1):
        ...
        if result["success"]:
            ...
            print(f"   📄 Text: {result['output_txt']}")
    ...
    print(f"Each PDF produced 2 files: .md (markdown) and .txt (plain text)")
...
def main():
    ...
    # 사용법 출력
    def print_usage():
        print("PDF to Markdown & Text Converter")
        print()
        print("Converts PDF to both .md (Markdown) and .txt (Plain Text) formats")
        print()
        print("Usage:")
    ...
    if len(sys.argv) == 1:
        ...
        if arg.endswith('.pdf'):
            ...
            if result["success"]:
                ...
                print(f"📄 Text: {result['output_txt']}")
                print(f"📊 Statistics:")
                print(f"   - MD Lines: {result['md_lines']}, Size: {result['md_size']:,} chars")
                print(f"   - TXT Lines: {result['txt_lines']}, Size: {result['txt_size']:,} chars")
                print(f"   - Tables: {result['tables']}, Images: {result['images']}")
            else:
